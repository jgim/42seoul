...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/09/20 22:10:49 by silim             #+#    #+#             */
...
void	builtin_cd(t_execdata *data)
{
	...
	data->elst = ft_setenv(data->elst, ft_custom_strdup("OLDPWD"), old_pwd, 0);
	if (ft_getenv(data->elst, "PWD"))
		data->elst = \
			ft_setenv(data->elst, ft_custom_strdup("PWD"), getcwd(NULL, 0), 0);
	g_status = 0;
}
