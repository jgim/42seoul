...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/09/20 22:11:29 by silim             #+#    #+#             */
...
static char	**make_exec_pathlist(char *cmd, char *path_env)
{
	...
	if (!path_env)
		return (NULL);
	pathlist = ft_custom_split(path_env, ':');
	i = 0;
	while (pathlist[i])
	{
		tmp = pathlist[i];
		pathlist[i] = ft_custom_strjoin(pathlist[i], "/");
		free(tmp);
		tmp = pathlist[i];
		pathlist[i] = ft_custom_strjoin(pathlist[i], cmd);
		free(tmp);
		i++;
	}
	return (pathlist);
...
static char	*cmdpath_from_pathenv(char *cmd, char *path_env)
{
	...
	while (pathlist && pathlist[++i])
	{
		...
		{
			cmd_path = ft_custom_strdup(pathlist[i]);
			break ;
		}
