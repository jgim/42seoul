...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/09/20 22:10:53 by silim             #+#    #+#             */
...
static t_envlist	*to_setenv(t_envlist *head, char *src_str, size_t i)
{
	...
	if (src_str[i] == '+')
	{
		key = ft_custom_substr(src_str, 0, i);
		value = ft_custom_substr(src_str, i + 2, ft_strlen(src_str) - i - 2);
		mode = 1;
	}
	else if (src_str[i] == '=')
	{
		key = ft_custom_substr(src_str, 0, i);
		value = ft_custom_substr(src_str, i + 1, ft_strlen(src_str) - i - 1);
		mode = 0;
	}
	else
	{
		key = ft_custom_strdup(src_str);
		value = NULL;
		mode = 0;
	}
