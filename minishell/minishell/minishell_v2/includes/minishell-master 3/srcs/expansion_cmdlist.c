...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/09/20 22:12:09 by silim             #+#    #+#             */
		...
		t_envlist *envlist, char *doll_ptr)
{
	...
	while (!is_delimiter(doll_ptr[len]))
		len++;
	front_key = ft_custom_substr(clist->str, 0, doll_ptr - clist->str);
...
static t_cmdlist	*insert_new_cmdlist(t_cmdlist *clist, size_t *i)
{
	...
	if (len == 0)
		return (clist);
	new = (t_cmdlist *)ft_custom_calloc(1, sizeof(*new));
	new->str = ft_custom_substr(clist->str, *i, len);
	tmp = clist->quot;
	new->quot = ft_custom_substr(clist->quot, *i, ft_strlen(clist->quot + *i));
	new->next = clist->next;
	clist->next = new;
	return (clist);
}
...
static void	search_new_space_cmdlist(t_cmdlist *clist)
{
	...
	if (i == 0 && ft_isspace(clist->str[i]))
	{
		while (ft_isspace(clist->str[i]))
			i++;
		str = ft_custom_substr(clist->str, i, ft_strlen(clist->str + i));
		free(clist->str), clist->str = str;
		tmp = clist->quot;
		clist->quot = ft_custom_substr(clist->quot, i, ft_strlen(clist->quot + i));
		free(tmp);
		search_new_space_cmdlist(clist);
	}
	else if (ft_isspace(clist->str[i]))
	{
		str = ft_custom_substr(clist->str, 0, i);
