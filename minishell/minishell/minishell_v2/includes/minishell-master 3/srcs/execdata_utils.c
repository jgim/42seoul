...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/09/20 22:11:44 by silim             #+#    #+#             */
...
/*
** stream and cmdlist have flag corresponding to each character of string.
** The flag meaning:
...
char	*get_quot_flag(char *str)
{
	...
	ret = (char *)ft_custom_calloc(ft_strlen(str) + 1, sizeof(char));
	i = 0;
	flag = '0';
	while (*str)
	{
...
t_cmdlist	*new_clst(t_cmdlist *cur, t_token *token)
{
	...
	cur->next = new;
	new->str = ft_custom_strdup(token->str);
