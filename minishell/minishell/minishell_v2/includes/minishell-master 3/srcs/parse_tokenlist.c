...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/09/20 22:12:39 by silim             #+#    #+#             */
...
static void	expand_status(t_token *list, char *doll_ptr)
{
	...
	front = ft_custom_substr(list->str, 0, doll_ptr - list->str);
	back = ft_custom_substr(doll_ptr, 2, ft_strlen(doll_ptr + 2));
	value = ft_custom_itoa(g_status);
	free(list->str);
