...
void init_player(t_player *player, t_data *data, int row, int col);
...
t_texture get_texture(t_data *map_data, t_ray *ray);
...
void player_direction(t_player *player, t_data *data, int i, int j);
void get_texture_image(t_data *map_data);
void turn_right_left(t_data *map_data, t_player *player);
void init_screen(t_data *map_data, t_texture *screen);
void calculate_rend_sprite(t_data *data, t_sprite *s, t_player *p, t_rayitem *r_item);
void *ft_memmove(void *dst, const void *src, int size);
void sort_sprite(t_data *map_data, t_sprite *sprite);
void fill_item_color(t_data *data, t_rayitem *r_item, t_texture img, t_texture *screen);
void free_factor_read(char **factor, int i);
void ft_free_map_rows(char **arr, int n);
int release_keys(int key_code, t_data *map_data);
...
int is_valid_cub(t_data *map_data, char *line)
{
	...
	if (*line == '1' || *line == ' ')
	{
		...
		return (map_data->parsing_error.data_type = 1);
	if (map_data->map_factor.read_map != 0)
...
int read_map(t_data *map_data, char *line)
{
	...
	if (map_data->map_factor.height_map == 0)
	{
		if (!(map_data->map_factor.map_line = malloc(sizeof(char *) * 2)))
			return (map_data->parsing_error.map_malloc = 1);
		map_data->map_factor.map_line[0] = ft_strdup(line);
		map_data->map_factor.map_line[1] = NULL;
	}
	else
	{
		if (!(temp = malloc(sizeof(char *) * (map_data->map_factor.height_map + 2))))
		{
			free_factor(map_data->map_factor.map_line);
			return (map_data->parsing_error.map_malloc = 1);
		}
...
void game_start(t_data *map_data)
{
	...
	mlx_hook(map_data->win_ptr, KEY_RELEASE_EVENT, 1L << 0, release_keys, map_data);
...
void set_screen(t_data *map_data)
{
	...
	map_data->ray.height = map_data->input_height;
...
int loop_game(t_data *map_data)
{
	...
	if (update)
	{
		// printf("loop_game, map_data->ray.cameraX : %f\n", map_data->ray.camera_x);
...
void init_screen(t_data *map_data, t_texture *screen)
{
	...
	screen->size_line = 0;
	screen->endian = 0;
	screen->img = mlx_new_image(map_data->mlx_ptr, map_data->ray.width, map_data->ray.height);
...
void set_ray(t_ray *ray, t_player *player)
{
	...
	// printf("set_ray->dirX : %f, plane[X] : %f , camera_x : %f\n", player->dir[X], player->plane[X], ray->camera_x);
	// printf("raywidth : %d\n", ray->width);
...
void set_raycasting(t_ray *ray, t_player *player)
{
	if (ray->ray_dir[Y] == 0)
		...
		ray->delta_dist[X] = (ray->ray_dir[X] == 0) ? 1 : fabs(1 / ray->ray_dir[X]);
	if (ray->ray_dir[X] == 0)
		ray->delta_dist[Y] = 0;
	else
		ray->delta_dist[Y] = (ray->ray_dir[Y] == 0) ? 1 : fabs(1 / ray->ray_dir[Y]);
	if (ray->ray_dir[X] < 0 && (ray->step[X] = -1))
		ray->side_dist[X] = (player->pos[X] - ray->map[X]) * ray->delta_dist[X];
	else if (ray->ray_dir[X] >= 0 && (ray->step[X] = 1))
		ray->side_dist[X] = (ray->map[X] + 1.0 - player->pos[X]) * ray->delta_dist[X];
	if (ray->ray_dir[Y] < 0 && (ray->step[Y] = -1))
		ray->side_dist[Y] = (player->pos[Y] - ray->map[Y]) * ray->delta_dist[Y];
	else if (ray->ray_dir[Y] >= 0 && (ray->step[Y] = 1))
...
{
	// printf("side : %d\n", ray->side);
	// printf("map[X] = %d, player->pos[X] : %f, ray->step[X] : %f, ray->ray_dir[X] : %f\n", ray->map[X], player->pos[X], ray->step[X], ray->ray_dir[X]);
	if (ray->side == 0)
...
void draw_line(t_data *map_data, t_line *line)
{
	line->height = (int)(map_data->ray.height / map_data->ray.perp_wall_dist);
	line->start = (-1 * line->height / 2) + (map_data->ray.height / 2);
	if (line->start < 0)
		line->start = 0;
	line->end = (line->height / 2) + (map_data->ray.height / 2);
	if (line->end >= map_data->ray.height)
		line->end = map_data->ray.height - 1;
	// printf("perpwalldist : %f\n", map_data->ray.perp_wall_dist);
	// printf("rayheight : %d, lineheight : %d\n", map_data->ray.height, line->height);
...
t_texture get_texture(t_data *map_data, t_ray *ray)
{
	...
	if (ray->side == 1)
	{
		if ((ray->ray_dir[X] <= 0 && ray->ray_dir[Y] <= 0) ||
			...
			texture = map_data->texture[2];
		else
			texture = map_data->texture[3];
	}
	else
	{
		if ((ray->ray_dir[X] <= 0 && ray->ray_dir[Y] <= 0) ||
			(ray->ray_dir[X] <= 0 && ray->ray_dir[Y] >= 0))
			texture = map_data->texture[0];
		else
			texture = map_data->texture[1];
	}
	return (texture);
}
...
void get_pixel_image(t_data *map_data, t_line line, t_texture *screen, t_texture wall_img)
{
	...
	while (++y < ray->height)
	{
		...
		else if (y >= line.start && y <= line.end)
		{
			...
			color = wall_img.data[wall_img.width * ray->tex[Y] + ray->tex[X]];
			screen->data[y * ray->width + ray->x] = color;
		}
...
void get_item(t_data *map_data, t_texture *screen)
{
	...
	sprite = map_data->sprite;
	player = &map_data->player;
	s_img = map_data->texture[4];
	i = -1;
	calculate_distance(map_data, sprite, player);
	sort_sprite(map_data, sprite);
	while (++i < map_data->map_factor.sprite_count)
	{
		calculate_rend_sprite(map_data, &sprite[i], player, &ray_item);
		fill_item_color(map_data, &ray_item, s_img, screen);
	}
}
...
void fill_item_color(t_data *data, t_rayitem *r_item, t_texture img, t_texture *screen)
{
	...
	x = r_item->start[X] - 1;
	while (++x < r_item->end[X])
	{
		img.tex[X] = (int)(256 * (x - (-r_item->width / 2 + r_item->screen_x)) * img.width / r_item->width) / 256;
		if (r_item->tf[Y] > 0 && x > 0 && x < data->ray.width &&
			r_item->tf[Y] < data->ray.buffer[X])
		{
			y = r_item->start[Y] - 1;
			while (++y < r_item->end[Y])
			{
				tmp = y * 256 - data->ray.height * 128 + r_item->height * 128;
				img.tex[Y] = ((tmp * img.height) / r_item->height) / 256;
				img.color = img.data[img.width * img.tex[Y] + img.tex[X]];
...
void calculate_rend_sprite(t_data *data, t_sprite *s, t_player *p, t_rayitem *r_item)
{
	r_item->pos[X] = s->pos[X] - p->pos[X] + 0.5;
	r_item->pos[Y] = s->pos[Y] - p->pos[Y] + 0.5;
	r_item->inv_det = 1.0 / (p->plane[X] * p->dir[Y] - p->dir[X] * p->plane[Y]);
	r_item->tf[X] = r_item->inv_det * (p->dir[Y] * s->center[X] - p->dir[X] * s->center[Y]);
	r_item->tf[Y] = r_item->inv_det * (-p->plane[Y] * s->center[X] + p->plane[X] * r_item->pos[Y]);
	r_item->screen_x = (int)((data->ray.width / 2) * (1 + r_item->tf[X] / r_item->tf[Y]));
	r_item->height = abs((int)(data->ray.height / r_item->tf[Y]));
	r_item->start[Y] = -r_item->height / 2 + data->ray.height / 2;
	if (r_item->start[Y] < 0)
		r_item->start[Y] = 0;
	r_item->end[Y] = r_item->height / 2 + data->ray.height / 2;
	if (r_item->end[Y] >= data->ray.height)
		r_item->end[Y] = data->ray.height - 1;
	r_item->width = abs((int)(data->ray.height / r_item->tf[Y]));
	r_item->start[X] = -r_item->width / 2 + r_item->screen_x;
	if (r_item->start[X] < 0)
		r_item->start[X] = 0;
	r_item->end[X] = r_item->width / 2 + r_item->screen_x;
	if (r_item->end[X] >= data->ray.width)
		r_item->end[X] = data->ray.width - 1;
}
...
void move_forward_backward(t_data *map_data, t_player *player)
{
	...
	i = 1;
	printf("key_value : %d\n", map_data->key_value);
	printf("ex : player->pos[X] : %f, player->pos[Y] : %f\n", player->pos[X], player->pos[Y]);
	printf("value[x][y] : %c\n", map_data->map_factor.map_line[(int)player->pos[X]][(int)player->pos[Y]]);
	if (map_data->key_value == S || map_data->key_value == DOWN)
		i = -1;
	printf("dir[x] : %f, dir[y] : %f\n", player->dir[X], player->dir[Y]);
	player->pos[X] += i * player->dir[X] * player->move_speed;
	player->pos[Y] += i * player->dir[Y] * player->move_speed;
...
void move_right_left(t_data *map_data, t_player *player)
{
	...
	if (map_data->key_value == A)
		i = -1;
	player->pos[X] += i * player->dir[Y] * player->move_speed;
	player->pos[Y] -= i * player->dir[X] * player->move_speed;
	if (map_data->map_factor.map_line[(int)player->pos[X]][(int)player->pos[Y]] != '0')
	{
		player->pos[X] -= i * player->dir[X] * player->move_speed;
		player->pos[Y] += i * player->dir[Y] * player->move_speed;
...
void turn_right_left(t_data *map_data, t_player *player)
{
	...
	// printf("turn : playerdirx : %f, playerdiry : %f", player->dir[X], player->plane[X]);
...
int bind_keys(int key_code, t_data *map_data)
{
	...
	if (key_code == LEFT || key_code == RIGHT)
	{
		printf("hit left right\n");
		map_data->key_value = key_code;
	}
	if (key_code == UP || key_code == W || key_code == DOWN || key_code == S)
	{
		printf("touch up down\n");
		map_data->key_value = key_code;
	}
	if (key_code == A || key_code == D)
	{
...
void get_texture_image(t_data *data)
{
	int i;
	...
	while (++i < 4)
	{
		data->texture[i].img = mlx_xpm_file_to_image(data->mlx_ptr, data->texture[i].path,
	...
	while (++i < map_data->map_factor.height_map)
	{
		...
		while (map_data->map_factor.map_line[i][++j])
		{
			if (ft_strchr("NSEW", map_data->map_factor.map_line[i][j]))
				init_player(&map_data->player, map_data, i, j);
...
void init_player(t_player *player, t_data *data, int row, int col)
{
	...
	player_direction(player, data, row, col);
}
...
void player_direction(t_player *player, t_data *data, int i, int j)
{
	...
	player->pos[Y] = j + 0.5;
	rotate = 0;
	if (data->map_factor.map_line[i][j] == 'S')
		rotate = M_PI;
	else if (data->map_factor.map_line[i][j] == 'E')
		rotate = M_PI * 0.5;
	else if (data->map_factor.map_line[i][j] == 'W')
		rotate = M_PI * 1.5;
	data->map_factor.map_line[i][j] = '0';
	tmp_dir[X] = player->dir[X];
	tmp_p[X] = player->plane[X];
	// printf("player->tmp[X]: %f ,player->tmp_p[X] : %f\n", tmp_dir[X], tmp_p[X]);
	player->dir[X] = tmp_dir[X] * cos(-rotate) - player->dir[Y] * sin(-rotate);
	player->dir[Y] = tmp_dir[X] * sin(-rotate) + player->dir[Y] * cos(-rotate);
	player->plane[X] = tmp_p[X] * cos(-rotate) - player->plane[Y] * sin(-rotate);
	player->plane[Y] = tmp_p[X] * sin(-rotate) + player->plane[Y] * cos(-rotate);
	// printf("player->dir[X]: %f ,player->dir[Y] : %f\n", player->dir[X], player->dir[Y]);
	// printf("player->plnae[X]: %f ,player->plane[Y] : %f\n", player->plane[X], player->plane[Y]);
}
